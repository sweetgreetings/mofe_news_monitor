...
STOPWORDS = {
    ...
    "비롯해", "비롯한", "비롯하여", "비롯",
    # [추가: 2026-08-10] "방안"은 한때 "정책 내용을 담을 수 있는 단어"라 일부러 빼뒀지만,
    # 실제 워드클라우드에서 "의견"·"발표"·"기준"·"확대"·"대책"·"대해"와 함께 상투적으로
    # 반복돼 정책 고유어(법률명·기관명 등)를 밀어내는 문제로 사용자가 직접 지적 — 매번
    # 설정 화면의 워드클라우드 제외어로 하나씩 등록하기보다, 소제목·키워드 추출 전체에
    # 공통으로 적용되는 이 목록으로 옮겼다.
    "방안", "의견", "발표", "기준", "확대", "대책", "대해",
}
...
# 이 패턴도 자동으로 함께 갱신된다.
#
# [수정: 2026-08-11] "것" 하나만 보던 걸 다른 의존명사까지 넓혔다 — 워드클라우드에
# "등을"·"등에" 같은 조사 잔재가 계속 올라온다는 사용자 지적 때문이다. 근본 원인은
# _strip_suffix의 안전장치(`len(word) > len(suffix) + 1`)다: "등을"은 "을"을 떼면 1글자만
# 남아서 조사가 안 떨어지고, 그대로 2글자 토큰으로 살아남는다. 안전장치 자체는
# "회의"→"회" 같은 오분리를 막는 것이라 건드릴 수 없어서, 애초에 뜻이 없는 의존명사
# 목록을 여기서 통째로 걸러내는 쪽으로 해결했다.
#
# 목록은 실제 저장된 기사 전체로 검증해 "이 글자+조사"가 진짜 단어와 겹치지 않는 것만
# 남겼다 — 예를 들어 "수"·"중"은 "수도"(수+도)·"중도"(중+도)처럼 뜻 있는 2글자 명사와
# 충돌해서 넣지 않았다.
_BOUND_NOUNS = ("것", "등", "때", "뿐", "만큼", "대로", "채", "탓")
_BOUND_NOUN_PATTERN = re.compile(
    r"^(" + "|".join(_BOUND_NOUNS) + r")(" + "|".join(_JOSA) + r")?$"
)
...
def tokenize(title: str, stopwords: set) -> set:
    ...
    for raw in _SPLIT_PATTERN.split(title):
        ...
        if word in stopwords:
            continue
        # [추가: 2026-08-10] "가"는 _JOSA에서 일부러 뺐다("경영평가"→"경영평" 오분리 사고,
        # 위 _JOSA 주석 참고) — 그 대가로 "재정경제부가"처럼 이미 불용어/제외어로 등록된
        # 단어 뒤에 "가"만 붙은 잔재가 그대로 워드클라우드에 남는 문제가 있었다. "가"를
        # 뗀 나머지가 이미 등록된 불용어와 정확히 일치할 때만 함께 걸러낸다 — "평가"처럼
        # "가"가 조사가 아니라 단어 일부인 경우는 그 나머지("평")가 불용어일 리 없어
        # 전혀 건드리지 않는다(기존 안전장치를 훼손하지 않는 일반화된 해법).
        if len(word) > 2 and word.endswith("가") and word[:-1] in stopwords:
            continue
        if (
            len(word) >= 2
