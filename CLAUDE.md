...
- **높이 네 단계**: 기사 행 안 `--h-sm`(24px), **툴바 버튼 `--h-tb`(28px)**, 입력칸 `--h-md`(30px, 기본), 폼의 대표 버튼·설정 화면 `--h-lg`(36px). 툴바 버튼은 좌우 여백 9px·굵기 500이고, **같은 줄의 「보기 순서」 select와 가져가기 글자도 같은 높이를 쓴다**(한 줄이 가지런해야 한다). 툴바를 28px로 내릴 때 입력칸까지 따라가지 않게 `--h-tb`를 따로 둔다 — 입력칸이 28px이면 글자 치기가 답답하다. **대표 단추(채움)의 높이는 그 폼·창 안 다른 칸에 맞춘다** — 새 수집의 `수집`은 옆 드롭다운과 같은 36px, 수시 편집 창의 `다시 수집`은 같은 창 `삭제`·`선택 해제`와 같은 28px(`.btn.sm`). 위계는 높이가 아니라 채움 색이 말한다. 시안 [mockups/BUTTON_COMPACT_MOCKUP.html](mockups/BUTTON_COMPACT_MOCKUP.html) ②안.
