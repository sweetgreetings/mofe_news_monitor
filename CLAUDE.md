...
- **끊는 순간 원래 회차 이름으로 저장된 기록을 새 회차 이름으로 복사한다**(`copy_round_records`): AI 분류 캐시·이름표·소제목 순서·키워드 메모(두 회차 모두)·자동 분류 1회 기록·재분류 실패 횟수, 초안에 보인 기사 목록은 **옮긴다**. 예약 회차가 수집되면 이어지는 회차로 다시 복사하고(`after_round_collected`, `main._scrape_and_render`), 취소하면 원래 회차로 되돌린다. **회차 이름으로 저장하는 새 저장소를 만들면 여기에 넣는다.**
...
- 「보기 순서」 select(소제목 내 언론사순/소제목 내 시간순/시간순/언론사순)는 **화면 전용**. 기본값 이름 `소제목 내 언론사순`은 출발 순서를 말한다 — 담당자가 ↑↓로 바꾼 순서도 이 보기에서 그대로 보인다.
...
- 화면: 검색어 편집 패널·다시 불러오기·수집 이력 칩 없음. 머리줄 맨 앞에 청록 `확정본` 칩(원본은 회색 `원본` 칩). 메타 칩 `←] 종부세 3건`(원본 링크, `icon("to_raw")` — 원본의 보내기 `[→`(`to_bundle`)와 짝, 이름은 저장된 `sent_from.report_title`). 좌하단 팝오버 이름은 「숨긴 기사」(조건 안내 문구 없음). 「모음 관리」 패널은 만들지 않는다. 원본에서 보내듯 확정본 → 확정본으로 **복사해 보내는** 길은 없다(서버도 거부) — 확정본끼리는 아래 「옮기기」만.
...
- **여섯 단계만 쓴다** — CSS엔 `font-size: var(--fs-md)`처럼 적고 숫자를 적지 않는다(정의는 모양 토큰과 같은 `SHAPE_TOKENS_CSS`):
  ...
  - `--fs-lg` 1.1rem: 소제목·카드 제목·날짜 줄·빈 화면 안내.
